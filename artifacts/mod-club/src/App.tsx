...
function Home({ session, onLogout, onSession }: { session: UserSession; onLogout: () => void; onSession: (session: UserSession) => void }) {
  ...
  const handleNav = (label: string) => {
    ...
    if (label === 'ODA AÇ' || label === 'Oyunlar') {
      setChatOpen(false);
      setActiveNav('ODA AÇ');
      setNotice('Odalar açık');
      return;
    }
    setChatOpen(false);
    setChatProfile(null);
  ...
  useEffect(() => {
    ...
    const syncKeyboard = () => {
      ...
      const inset = typing && viewport ? Math.max(0, window.innerHeight - viewport.height - viewport.offsetTop) : 0;
  ...
  useEffect(() => {
    ...
    const onVisible = () => {
      if (!document.hidden) window.setTimeout(thaw, 180);
    };
                        ...
                        return (
                          <div key={member.id} className="admin-row">
                            ...
                            <div className="flex flex-wrap items-center gap-1.5">
                              ...
                              {member.role === 'ADMIN' ? (
                                <span className="rounded-full bg-[hsl(var(--secondary))] px-2 py-1 font-mono text-[.48rem] font-bold text-[hsl(var(--primary))]">Oda paketi sınırsız</span>
                              ) : (
